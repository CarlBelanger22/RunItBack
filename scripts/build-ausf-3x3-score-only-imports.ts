/**
 * Build AUSF 3x3 score-only import JSON bundles (LE-128).
 *
 * Usage:
 *   npx tsx scripts/build-ausf-3x3-score-only-imports.ts
 *   npx tsx scripts/build-ausf-3x3-score-only-imports.ts --dry-run
 */
...
import {
  ...
  TEAM_META,
  SCORE_ONLY_GAMES,
  type AusfScoreOnlyGameDef,
  type AusfTeamId,
} from './ausf-3x3-schedule-data';

const OUT_DIR = join(resolve(process.cwd(), 'Importingboxscores', 'AUSF 3x3'), 'json');

function teamBundleRow(teamId: AusfTeamId) {
...
async function main(): Promise<void> {
  ...
  // Only list teams already in DB/enrolled. New teams are created + enrolled
  // when a game featuring them is imported (home/away auto-enroll).
  ...
  console.log(
    `Building ${SCORE_ONLY_GAMES.length} AUSF 3x3 score-only bundles…\n`
  );
  ...
  if (dryRun) {
    ...
    console.log('Import all: npm run import:ausf-3x3-score-only');
  }
}
