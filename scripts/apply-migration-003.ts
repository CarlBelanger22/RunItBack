/**
 * Apply C11 migration (global position on players).
...
function main() {
  ...
  const migrationPath = resolve(
    process.cwd(),
    'supabase/migrations/003_player_global_position.sql'
  );

  console.log('RunItBack - migration 003 (global player position)\n');
  ...
  if (!dbUrl) {
    ...
    console.log('  2. Paste: supabase/migrations/003_player_global_position.sql');
  ...
  try {
    ...
    console.log('\nMigration 003 applied. Hard-refresh the app.');
  } catch {
