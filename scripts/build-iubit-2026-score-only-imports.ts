/**
 * Build IUBIT 2026 score-only import JSON bundles (27 games).
 *
 * Usage:
 *   npx tsx scripts/build-iubit-2026-score-only-imports.ts
 *   npx tsx scripts/build-iubit-2026-score-only-imports.ts --dry-run
 */
...
import {
  ...
  TEAM_META,
  SCORE_ONLY_GAMES,
  type IubitScoreOnlyGameDef,
  type IubitTeamId,
} from './iubit-2026-schedule-data';

const OUT_DIR = join(resolve(process.cwd(), 'Importingboxscores', 'IUBIT 2026'), 'json');

function teamBundleRow(teamId: IubitTeamId) {
...
async function main(): Promise<void> {
  ...
  for (const id of PROTECTED_GAME_IDS) {
    if (!existingGameIds.has(id)) {
      console.warn(`Warning: protected game ${id} not found in Supabase (expected existing stat game)`);
    }
  }
  ...
  console.log(`Building ${SCORE_ONLY_GAMES.length} IUBIT 2026 score-only bundles…\n`);
  ...
  for (const game of SCORE_ONLY_GAMES) {
    ...
    console.log(
      `M${String(game.matchNo).padStart(2, '0')} | ${game.id} | ${homeMeta.abbreviation} ${game.homeScore}-${game.awayScore} ${awayMeta.abbreviation} | ${game.date} ${game.startTime}`
    );
  }
  ...
  if (dryRun) {
    ...
    console.log(
      'Import all: npm run import:iubit-2026-score-only'
    );
    console.log(
      'Or one game: npm run import:boxscore -- --file "<path>" --stats-only'
    );
  }
}
