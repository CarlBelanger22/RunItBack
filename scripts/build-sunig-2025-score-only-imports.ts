/**
 * Build SUniG 2025 score-only import JSON bundles (LE-110).
 *
 * Usage:
 *   npx tsx scripts/build-sunig-2025-score-only-imports.ts
 *   npx tsx scripts/build-sunig-2025-score-only-imports.ts --dry-run
 */
...
import {
  ...
  ALL_TEAM_IDS,
  SCORE_ONLY_GAMES,
  type SunigScoreOnlyGameDef,
  type SunigTeamId,
} from './sunig-2025-schedule-data';

const OUT_DIR = join(resolve(process.cwd(), 'Importingboxscores', 'sunig 2025'), 'json');

function teamBundleRow(teamId: SunigTeamId) {
...
async function main(): Promise<void> {
  ...
  console.log(`Building ${SCORE_ONLY_GAMES.length} SUniG 2025 score-only bundles…\n`);
  ...
  for (const game of SCORE_ONLY_GAMES) {
    ...
    console.log(
      `${game.phase.padEnd(14)} | ${game.id} | ${homeMeta.abbreviation} ${game.homeScore}-${game.awayScore} ${awayMeta.abbreviation} | ${game.date} ${game.startTime}${game.note ? ` (${game.note})` : ''}`
    );
  }
  ...
  if (dryRun) {
    ...
    console.log('Import all: npm run import:sunig-2025-score-only');
  }
}
