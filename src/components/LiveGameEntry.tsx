...
type DialogActionType = 'free_throw' | 'foul' | 'turnover' | 'substitution' | 'rebound';
...
export function LiveGameEntry({ game, onGameUpdate, onGameComplete }: LiveGameEntryProps) {
  ...
  const startAction = (type: ActionType, player?: Player) => {
    ...
    } else if (['free_throw', 'foul', 'turnover', 'substitution', 'rebound'].includes(type)) {
      setDialogState({ isOpen: true, type: type as DialogActionType | 'rebound' });
    }
  };
  ...
  const recordEvent = (eventData: Partial<GameEvent>) => {
    const event: GameEvent = {
      ...
      teamId: eventData.teamId || ourTeam.id,
      playerId: eventData.playerId || actionState.selectedPlayer?.id,
      details: eventData.details || {},
      homeScore: ourScore,
      awayScore: opponentScore,
    };

    const updatedGame = GameLogic.recordEvent(currentGame, event);

    setCurrentGame(updatedGame);
    onGameUpdate(updatedGame);
    
    // Add to undo stack for local UI (optional since GameLogic handles it in events)
  ...
  const handleUndo = () => {
    ...
    const updatedGame = GameLogic.undoLastEvent(currentGame);
  ...
  const PlayerChip = ({ player, isStarter = false }: { player: Player; isStarter?: boolean }) => {
    ...
    const stats: any[] = [];
    if (playerFouls > 0) {
      stats.push({ value: playerFouls, label: 'F', variant: 'destructive' });
    }
    ...
    return (
      <PlayerCard
        player={player}
        isStarter={isStarter}
        selected={actionState.selectedPlayer?.id === player.id}
              ...
              <div className="flex gap-2">
                <Button 
                  onClick={() => {
                    ...
                    // Update shots array first (or we could move this into GameLogic)
                    const gameWithShot = {
                      ...currentGame,
                    ...
                    const eventData: Partial<GameEvent> = {
                      ...
                      details: {
                        ...
                        assistedBy: shot.assistedBy,
                        blockedBy: shot.blockedBy,
                        isTransition: shot.isTransition,
                        inPaint: shot.inPaint,
                        x: shot.x,
                        y: shot.y
                      }
                    };
                    
                    // Record event (this updates stats and events)
                    const updatedGame = GameLogic.recordEvent(gameWithShot, {
                      id: `event-${Date.now()}`,
                      type: 'shot_attempt',
      ...
      <div className="flex-1 flex">
        ...
        <div className="flex-1 p-4 space-y-4">
          ...
          <CourtView
            ...
            onCourtClick={(x, y) => {
              if (actionState.type !== 'shot') return;
              
              // Determine if shot is in paint based on position
              const inPaint = x >= 35 && x <= 65 && y >= 65;
              
              // Determine if shot is beyond three-point line
              const centerX = 50;
              const centerY = 85; 
              const distance = Math.sqrt(Math.pow(x - centerX, 2) + Math.pow(y - centerY, 2));
              const isThree = distance > 25;
              ...
              setActionState(prev => ({
                ...
                data: { 
                  ...
                  y, 
                  inPaint,
                  suggestedThreePoint: isThree
                }
              }));
            }}
            showZones={showZones}
            interactive={actionState.type === 'shot'}
          />
        ...
        <div className="w-80 border-l p-4 space-y-4">
          ...
          <Card className="flex-1 overflow-hidden flex flex-col">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">Game Feed</CardTitle>
            </CardHeader>
                ...
                <StatActionButton 
                  ...
                  onClick={() => {
                    recordEvent({
                      type: 'shot_attempt',
                      teamId: opponentTeam.id,
                      details: { made: true, isThree: true }
                    });
                  }}
                />
              </div>
                ...
                <StatActionButton 
                  ...
                  onClick={() => {
                    recordEvent({
                      type: 'shot_attempt',
                      teamId: opponentTeam.id,
                      details: { made: false, isThree: false }
                    });
                    setAwaitingRebound(true);
                  }}
                />
        ...
        onConfirm={(data) => {
          ...
          const eventData: Partial<GameEvent> = {
            type: dialogState.type === 'free_throw' ? 'free_throw' : 
                  ...
                  dialogState.type === 'turnover' ? 'turnover' : 
                  dialogState.type === 'rebound' ? 'rebound' : 'substitution',
            playerId: dialogState.type === 'rebound' ? data.playerId : actionState.selectedPlayer?.id,
            details: data,
            teamId: dialogState.type === 'rebound' 
              ? (data.reboundType === 'offensive' ? ourTeam.id : opponentTeam.id)
              : ourTeam.id
          };
