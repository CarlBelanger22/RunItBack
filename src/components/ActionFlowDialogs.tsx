...
interface ActionFlowDialogsProps {
  isOpen: boolean;
  onClose: () => void;
  actionType: 'free_throw' | 'foul' | 'turnover' | 'substitution' | 'rebound' | null;
...
const ReboundDialog = React.memo(({ ourTeam, opponentTeam, formData, setFormData, onClose, onConfirm }: { ourTeam: Team; opponentTeam: Team; formData: any; setFormData: React.Dispatch<React.SetStateAction<any>>; onClose: () => void; onConfirm: () => void; }) => (
  <DialogContent>
    <DialogHeader>
      <DialogTitle>Record Rebound</DialogTitle>
      <DialogDescription>
        Select the player who secured the rebound
      </DialogDescription>
    </DialogHeader>
    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="grid grid-cols-2 gap-2 mt-2">
          <Button
            variant={formData.reboundType === 'offensive' ? "default" : "outline"}
            onClick={() => setFormData(prev => ({ ...prev, reboundType: 'offensive' }))}
          >
            Offensive
          </Button>
          <Button
            variant={formData.reboundType === 'defensive' ? "default" : "outline"}
            onClick={() => setFormData(prev => ({ ...prev, reboundType: 'defensive' }))}
          >
            Defensive
          </Button>
        </div>
      ...
      {formData.reboundType && (
        <div>
          <Label>Player</Label>
          <div className="grid grid-cols-2 gap-2 mt-2 max-h-48 overflow-y-auto p-1">
            <Button
              variant={formData.playerId === 'team' ? "default" : "outline"}
              onClick={() => setFormData(prev => ({ ...prev, playerId: 'team' }))}
              className="text-xs"
            >
              Team Rebound
            </Button>
            {(formData.reboundType === 'offensive' ? ourTeam : opponentTeam).players.map(player => (
              <Button
                key={player.id}
                variant={formData.playerId === player.id ? "default" : "outline"}
                onClick={() => setFormData(prev => ({ ...prev, playerId: player.id }))}
      ...
      <div className="flex justify-end space-x-2">
        ...
        <Button 
          onClick={onConfirm}
          disabled={!formData.reboundType || !formData.playerId}
        >
          Record Rebound
        </Button>
      </div>
...
ReboundDialog.displayName = 'ReboundDialog';
